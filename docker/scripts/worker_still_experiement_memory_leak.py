...
SILENCE_TIMEOUT = 5
USE_NNET2 = False
#global_model_yaml = None
#global_decoder_pipeline = None
...
class ServerWebsocket(WebSocketClient):
    ...
    def __init__(self, uri):
        ...
        uri = uri + '?%s' % (urllib.urlencode([("worker-model", "AZURE_STORAGE_ACCOUNT_"+ os.getenv('AZURE_CONTAINER')  )]))
        logger.info('uri='+ uri)
    ...
    def opened(self):
        ...
        self.ac1 = [a1**5 for a1 in xrange(20000000)]
    ...
    def load_model(self, model_yaml):
            #global global_model_yaml
            # speed up if subsequent request use the same model
            # if global_model_yaml == model_yaml:
            #     logger.info('re-use model %s' % model_yaml)
            #     self.decoder_pipeline = global_decoder_pipeline 
            #     return
            # else:
            #     logger.info('load model %s for the first time' % model_yaml)
            ...
            if USE_NNET2:
                self.decoder_pipeline = DecoderPipeline2(conf)
            else:
                self.decoder_pipeline = DecoderPipeline(conf)
    ...
    def received_message(self, m):
        ...
        if self.state == self.__class__.STATE_CONNECTED:
            ...
            self.num_segments = 0
             ...
             # check model_yaml file really exist otherwise, disconnect and reconnect
            if not os.path.exists(model):
                logger.warning('model_yaml path=%s does not exist ' % model)
                self.finish_request()
                event = dict(status=common.STATUS_NO_SPEECH)
                try:
                    self.send(json.dumps(event))
            ...
            self.load_model(model_yaml=model) # load '/opt/models/SingaporeEnglish_1018NNET3/engine.yaml'
            ...
            # TODO: check memory usage because create DecoderPipeline each time

            #self.decoder_pipeline.asr.set_property('model', model) # '/opt/models/final_test.mdl'
            #logger.info('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>'+ str(self.decoder_pipeline.asr.get_property('model') ))
...
# class ServerWebsocket_2(WebSocketClient):

#     def __init__(self, uri):     
#         WebSocketClient.__init__(self, url=uri, heartbeat_freq=10)

#     def opened(self):
#         logger.info('openining  ServerWebsocket_2')
#     def received_message(self, m):
...
def main_loop(uri):
    ...
    #while numberProcessed > 0:
        ...
        ws = ServerWebsocket(uri)
        # ##ws = ServerWebsocket_2(uri)

        try:
            logger.info("Opening websocket connection to master server testing")
            ws.connect()
            ws.run_forever()
            #numberProcessed -=1
    ...
    #    ac1 = [a1**5 for a1 in xrange(30000000)]
    #    #del ac1
    #    logger.info('done >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
