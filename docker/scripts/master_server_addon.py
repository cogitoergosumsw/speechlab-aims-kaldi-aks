...
def create_job(MODEL):
    ...
    container = client.V1Container(name= '{}-c'.format(name),
                                    ...
                                    env=env_list,
                                    security_context=client.V1SecurityContext(privileged=True, capabilities=client.V1Capabilities(add=["SYS_ADMIN"])),
                                    resources=client.V1ResourceRequirements(limits={"memory":"5G","cpu": "1"}, requests={"memory":"5G", "cpu": "1"} )
                                    )
    template.template.spec = client.V1PodSpec(containers=[container],
                                                image_pull_secrets= [{ "name":"azure-cr-secret"}],
                                                restart_policy="OnFailure" #reason to use OnFailure https://github.com/kubernetes/kubernetes/issues/20255
                                                )
